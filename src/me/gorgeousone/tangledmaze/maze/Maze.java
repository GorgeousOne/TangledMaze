...
public class Maze {
	...
	public void toggleExit(Block block) {
		...
		if(!getClip().borderContains(newExit)) {
			return;
		}
		
		if(!canBeExit(block)) {
			Utils.sendBlockDelayed(getPlayer(), block.getLocation(), Constants.MAZE_BORDER);
			return;
		}
		...
		if(isExit(block)) {
			...
			Utils.sendBlockDelayed(getPlayer(), newExit, Constants.MAZE_BORDER);
			...
			exits.add(newExit);
			Utils.sendBlockDelayed(getPlayer(), newExit, Constants.MAZE_MAIN_EXIT);
		}
...
//			
//			MazePoint exit = exits.get(i);
//			
//			if(!exit.equals(point) || exit.getY() != point.getY()) {
//				continue;
//			}
//			
//			//TODO remove
//			Bukkit.broadcastMessage(exit.toString() + ", " + point.toString());
//			
//			if(exits.size() > 1 && exit.equals(getMainExit()))
...
//	}
	...
	private void expandBorder(MazePoint point, MazeAction changes) {
		...
		for(Directions dir : Directions.values()) {
			...
			MazePoint point2 = Utils.nearestSurface(point.clone().add(dir.toVec3()));
	...
	public boolean sealsMaze(Location loc, MazeAction changes, Directions[] directions) {
		...
		for(Directions dir : directions) {
			...
			MazePoint point2 = new MazePoint(loc.clone().add(dir.toVec3()));
